...
namespace D3Demos.Controllers
{
    ...
    public class TagDataPointController : ApiController
    {
        ...
        {
            return dataPoints.OrderBy(m => m.Place).ThenBy(m=> m.Address).ToList();
        }
        ...
        {
            var items = dataPoints.Where(m=>m.Place == tag)
                                  .OrderBy(m => m.Address)
                                  .ToList();
        ...
        private List<TagDataPoint> GetSOData() {
            var path = Path.Combine(HttpContext.Current.Request.PhysicalApplicationPath, @"Data\TaiwanCenter.json");
